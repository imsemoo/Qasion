...
**قاعدة البرتقالي:** برتقالي مصمت = زر/فعل. برتقالي نصي صغير (<18px) عند الربط الفعلي بالإنتاج يُستبدل بـ`#C8501A` لضمان تباين WCAG AA.
...
| Ticker | marquee مستنسخ ×2 (`-50%` loop) — **لا يتوقف** مع reduced-motion (محتوى)، يقف بالـhover (WCAG 2.2.2) |
...
### الرئيسية (`pages.css`)
ticker (marquee) · update-card (newswire bar) · featured + issue-card (hero) · assess + wire (مرقّمة) · **dossier** (tabs تفاعلية) · trans-card (اقتباس) · maps + data-tile · brief (غلاف+محتوى) · mostread (دوائر مرتّبة) · **agenda-card** (تقويم) · **pub-card** (إصدار بكعب + hover قلب كامل) · special (بانر) · zz-row (زجزاج) · **monitor** (غرفة رصد: ساعة + progress يوم + أيقونات تصنيف) · **versus** (مواجهة) · **offers** (بلاطات أيقونات) · **bignums** (عدّادات) · newsletter
...
| ticker | `index.html .ticker__group` | ضع أي عدد عناصر — الاستنساخ تلقائي |
...
| البحث | حالياً decorative | الربط المقترح: `research.html?q=` |
